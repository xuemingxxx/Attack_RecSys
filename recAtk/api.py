...
import transformers
import recAtk
from recAtk.models.model_utils import device

SUPPORTED_MODELS = ["text-embedding-ada-002", "gtr-base"]


def load_pretrained_corrector(embedder: str) -> recAtk.trainers.Corrector:
    ...
    if embedder == "text-embedding-ada-002":
        inversion_model = recAtk.models.InversionModel.from_pretrained(
            "jxm/recAtk__openai_ada002__msmarco__msl128__hypothesizer"
        )
        model = recAtk.models.CorrectorEncoderModel.from_pretrained(
            "jxm/recAtk__openai_ada002__msmarco__msl128__corrector"
        )
...
def load_corrector(
    inversion_model: recAtk.models.InversionModel,
    corrector_model: recAtk.models.CorrectorEncoderModel,
) -> recAtk.trainers.Corrector:
    """Load in the inversion and corrector models

    Args:
        inversion_model (recAtk.models.InversionModel): _description_
        corrector_model (recAtk.models.CorrectorEncoderModel): _description_

    Returns:
        recAtk.trainers.Corrector: Corrector model to invert an embedding back to text
    """

    inversion_trainer = recAtk.trainers.InversionTrainer(
    ...
    corrector = recAtk.trainers.Corrector(
        model=corrector_model,
        inversion_trainer=inversion_trainer,
        args=None,
        data_collator=recAtk.collator.DataCollatorForCorrection(
