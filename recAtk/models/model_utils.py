...
def load_collm_model(ckpt_path, is_movie):
    ...
    task = setup_task(cfg)
    # d_path = 'data/datasets/collm_datasets/movie/train/mini_collm_movie_train.json'
...
def get_max_user_and_item_id_from_file(json_path):
    ...
    item_list_pattern = re.compile(r'IDs?:\s*([0-9,\s]+)')  # 历史喜欢的 item ID
    item_single_pattern = re.compile(r'titled\s+\\".*?\\" with its ID (\d+)')  # 被问询的 item ID
