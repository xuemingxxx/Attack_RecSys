...
class BaseTrainer(transformers.Trainer):
    ...
    def eval_generation_metrics(
        ...
        """
        # Get a sample of predicted and ground truth sequences
        ...
        # Calculate text-based similarity metrics like BLEU, ROUGE, F1
        ...
        # Print a few prediction-label examples to console for sanity check
        ...
        # Convert prediction/label samples to tensors and truncate to 128 examples
        ...
        # Calculate average number of non-padding, non-BOS tokens in predictions and labels
        num_tokens_metrics = {
            "pred_num_tokens": (
        ...
        # Pad predictions with EOS token (if applicable)
        ...
        try:
            with torch.no_grad():
                # Retokenize predicted strings with the embedder tokenizer
                ...
                # Retokenize labels and compute their embeddings
        ...
        def extract_items(texts):
            """Extract quoted item names from each text string, removing invalid responses like "yes" or "no"."""
        ...
        def extract_items_list(texts):
            """imilar to extract_items, but preserves the order of appearance and removes duplicates while preserving order"""
            item_pattern = re.compile(r'"([^"]*?)(?:\s\(\d{4}\))?"')
            all_items = []
            for t in texts:
                candidates = item_pattern.findall(t)
        ...
        def extract_profile(text):
            """Extract age and gender from the input text based on common patterns."""
            age = None
            gender = None
            text_lower = text.lower()
            matches = list(re.finditer(r'"([^"]+)"', text_lower))

            # Define the region to search for age/gender as the part before the first actual item quote.
        ...
        # Extract item sets and lists from predictions and ground truths
        ...
        # ---------- 1. item_match ----------
        # Compute the ratio of correctly predicted items (intersection / true set size)
        match_ratios = []
        ...
        # ---------- 3. profile_match  ----------
        # Compare extracted age and gender from prediction vs. label
        profile_matches = []
        ...
        # Ensure all metric values are standard Python types (not numpy types)
        for k, v in metrics.items():
