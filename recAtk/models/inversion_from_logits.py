...
from recAtk.models.inversion import InversionModel
...
class InversionFromLogitsModel(InversionModel):
    """
    An inversion model that reconstructs input prompts from logits by learning
    a transformation over the embedding space. 
    """
    def __init__(self, config: InversionConfig):
        super().__init__(config=config)

        # Get hidden size from encoder-decoder backbone (e.g., T5)
        encoder_hidden_dim = self.encoder_decoder.config.hidden_size
        self.encoder_hidden_dim = encoder_hidden_dim
        self.embedder_is_decoder = True
        bottleneck_dim = self.bottleneck_dim

        # Compute how many padding zeros are needed to align vocab+pad to hidden dim
        if self.embedder.__class__.__name__ == "MiniGPT4Rec_v2":
            self.num_zeros_to_add = encoder_hidden_dim - (
                (self.embedder.llama_model.config.vocab_size + encoder_hidden_dim) % encoder_hidden_dim
            )
            self.num_repeat_tokens = round(
                (self.embedder.llama_model.config.vocab_size + self.num_zeros_to_add)
                / encoder_hidden_dim
            )
        else:
            # General case for standard Hugging Face-like models
            self.num_zeros_to_add = encoder_hidden_dim - (
        ...
        # Embedding transformation network (bottleneck projection and recovery)
        ...
        # Learnable weights for each token position, to align sequence embeddings with logits
        ...
        if self.embedder.__class__.__name__ == "MiniGPT4Rec_v2":
            self.unigram = nn.Parameter(
                torch.zeros(
                    (1, self.embedder.llama_model.config.vocab_size + self.num_zeros_to_add),
                    dtype=torch.float32,
                ),
                requires_grad=False,
            )
        ...
        # Softmax sampling configuration (can be set during inference/training)
    ...
    def call_embedding_model(
        ...
        # if self.embedder.__class__.__name__ == "MiniGPT4Rec_v2":
        # model_output = self.embedder(input_ids=input_ids)
        ...
        # Re-tokenize the text using the embedder's tokenizer
    ...
    def _process_embedder_output(
        ...
        if self.embedder.__class__.__name__ == "MiniGPT4Rec_v2":
            ...
            last_token_indices = (attention_mask.sum(1).long() - 1).clamp(min=0, max=outputs.logits.size(1) - 1)
            indices = torch.arange(B, device=outputs.logits.device)
            logits = outputs.logits[indices, last_token_indices]
        else:
