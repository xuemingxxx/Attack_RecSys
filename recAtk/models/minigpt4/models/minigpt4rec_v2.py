...
class MiniGPT4Rec_v2(Rec2Base):
    ...
    def recprompt_wrap_v2(self, samples, ori_samples, atts_sample, prompt): # used for stage 2
        if prompt:
            ...
            if "<UserID>" in prompt_ori  and "<ItemIDList>" in prompt_ori and  "<TargetItemID>" in prompt_ori:
                ...
                prompt_embeds[replaced_idx[:,0],replaced_idx[:,1]] = torch.cat([samples['User_emb'], samples['TargetItem_emb']],dim=-2).reshape(-1,samples['User_emb'].shape[-1]).type_as(prompt_embeds)
    ...
    #             # 解析目标物品ID：同样处理占位符情况
    #             m = re.search(r'with (?:its|the)\s+ID\s+(\d+)', prompt)
    #             if m:
    #                 tid = int(m.group(1))
    ...
    #             m = re.search(r'following (?:movies|books):\s*(.*?)\.', prompt, re.DOTALL | re.IGNORECASE)
    ...
    #             m = re.search(r'(?:movie|book) titled\s*"([^"]+)"', prompt, re.IGNORECASE)
