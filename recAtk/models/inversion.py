...
from recAtk.models.model_utils import (
    FREEZE_STRATEGIES,
    disable_dropout,
    freeze_params,
...
from recAtk.utils import embed_api
...
class InversionModel(transformers.PreTrainedModel):
    ...
    def __init__(self, config: InversionConfig):
        ...
        if embedder_model_api:
            ...
            bottleneck_dim = self.embedder_dim
        elif embedder.__class__.__name__ == "MiniGPT4Rec_v2":
            # if embedder is collm，use the hidden_size of the inside LLaMA model as embedder_dim
            self.embedder_dim = embedder.llama_model.config.hidden_size
            bottleneck_dim = self.embedder_dim
        else:
