...
from recAtk.models.inversion_from_logits import InversionFromLogitsModel
from recAtk.tokenize_data import get_tokenizer_mapping
...
class InversionFromLogitsEmbModel(InversionFromLogitsModel):
    def __init__(self, config: InversionConfig):
        ...
        if self.embedder.__class__.__name__ == "MiniGPT4Rec_v2":
            self.embedder_vocab_size = self.embedder.llama_model.config.vocab_size
        else:
    ...
    def embed_and_project(
        ...
        """Computes embeddings for input tokens and projects them to a token-level representation space
        for use in prompt reconstruction or generation."""

        # Use externally provided frozen embeddings if available
        if frozen_embeddings is not None:
            embeddings = frozen_embeddings
            assert len(embeddings.shape) == 2  # batch by d
        elif self.embedder_no_grad:
            # If embedder is frozen, compute embeddings without gradient tracking
            with torch.no_grad():
            ...
            # Default: compute embeddings with gradient flow
            embeddings = self.call_embedding_model(
                input_ids=input_ids,
                attention_mask=attention_mask,
            )
        ...
        # Expand the tokenizer mapping to shape (batch_size, vocab_size)
        mapping = (
        ...
        # Project embeddings into vocab space using soft-counting via scatter_add
        # Use exponential then log to simulate log-space accumulation
        ...
        # If in training mode, update running unigram distribution (used for bias correction)
        if self.training:
        ...
        # Add zero-padding tokens to ensure divisibility with hidden dimension
        ...
        # Reshape logits into (batch_size, num_tokens, hidden_dim)
        ...
        # Sanity check: shape should match (batch_size, num_tokens, hidden_dim)
