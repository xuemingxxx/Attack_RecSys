...
import recAtk
from recAtk.collator import DataCollatorForCorrection
from recAtk.data_helpers import dataset_from_args, load_standard_val_datasets
from recAtk.models import (
    InversionFromLogitsEmbModel,
    InversionFromLogitsEmbModelIterative,
...
from recAtk.tokenize_data import (
    embed_dataset_batch,
    tokenize_function,
    tokenize_function_llama_chat,
)
from recAtk.utils import MockEmbedder, dataset_map_multi_worker, get_num_proc
...
#     "recAtk_CACHE", os.path.expanduser("~/.cache/inversion")
# )
...
class Experiment(abc.ABC):
    ...
    def _load_train_dataset_uncached(
        ...
        print('We are in _load_train_dataset_uncached')
    ...
    def _load_val_datasets_uncached(
        ...
        val_datasets_dict = load_standard_val_datasets(testset_name)
        logger.info(
    ...
    def load_train_and_val_datasets(
        ...
        print('We are in load_train_and_val_datasets()')
        ...
        train_dataset_path = os.path.join(
            DATASET_CACHE_PATH, (self.data_args.dataset_name + str(self.data_args.use_less_data) + ".arrow")
        )
        ...
        ######################################################################
        testset = self.data_args.testset_name
        if testset is not None:
            val_dataset_path = os.path.join(
                DATASET_CACHE_PATH, (testset + "_valid.arrow")
            )
        else:
            val_dataset_path = os.path.join(
                DATASET_CACHE_PATH, (self.data_args.dataset_name.replace("train", "valid") + ".arrow")
            )
...
class InversionExperiment(Experiment):
    ...
    def trainer_cls(self):
        return recAtk.trainers.InversionTrainer
...
class InversionFromLogitsExperiment(InversionExperiment):
    @property
    def trainer_cls(self):
        return recAtk.trainers.InversionFromLogitsTrainer
    ...
    def load_model(self) -> transformers.PreTrainedModel:
        if self.training_args.experiment == "inversion_from_logits_emb":
            print('We are in load_model(), loading inversion_from_logits_emb')
            return InversionFromLogitsEmbModel(config=self.config)
        elif self.training_args.experiment == "inversion_from_logits_emb_iterative":
            # import ipdb
            # ipdb.set_trace()
            print('We are in load_model(), loading inversion_from_logits_emb_iterative')
            return InversionFromLogitsEmbModelIterative(config=self.config)
        else:
...
class InversionExperimentNonAutoregressive(Experiment):
    ...
    def load_trainer(self) -> transformers.Trainer:
        ...
        return recAtk.trainers.InversionTrainerNonAutoregressive(
            model=model,
            args=self.training_args,
            train_dataset=train_dataset,
            eval_dataset=eval_dataset,
...
class InversionExperimentBagOfWords(Experiment):
    ...
    def load_trainer(self) -> transformers.Trainer:
        ...
        return recAtk.trainers.InversionTrainerBagOfWords(
            model=model,
            args=self.training_args,
            train_dataset=train_dataset,
            eval_dataset=eval_dataset,
    ...
    def load_trainer(self) -> transformers.Trainer:
        if self.training_args.corrector_model_from_pretrained:
            (
                _,
                inversion_trainer,
            ) = recAtk.analyze_utils.load_experiment_and_trainer_from_pretrained(
...
EXPERIMENT_CLS_MAP = {
    ...
    "inversion_from_logits_emb_iterative": InversionFromLogitsExperiment,
